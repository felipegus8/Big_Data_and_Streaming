...
public class TweetManager implements LifecycleManager, Serializable {
    ...
    StatusListener listener = new StatusListener() {
        public void onStatus(Status status) {
            Tweet newTweet = new Tweet(status.getId(),status.getUser().getName(),status.getText(),
                    status.getCreatedAt(),status.getSource(),status.isTruncated(),
                    status.isFavorited(),status.getGeoLocation(),status.getLang(),status.getContributors(),
